...
const CartButton = ({ product }) => {
  ...
  useEffect(() => {
    ...
    if (cartError?.data) {
      ...
      // If error is auth-related, redirect to signin and save product info
      if (
        cartError?.data?.description?.toLowerCase().includes("sign in") ||
        cartError?.data?.description?.toLowerCase().includes("unauthorized") ||
        cartError?.status === 401
      ) {
        localStorage.setItem(
          "pendingAddToCart",
          JSON.stringify({ product: product._id, quantity: qty })
        );
